...
def run_neural_interpolation(image1_name, image2_name, n_intervals=5, uid='s1', save_path = '/home/gru/akshay/synthesis/neurint', num_steps=1000,
                             layer='pool4', ver='V6', area='IT', input_image_dir='/home/gru/akshay/textures/input', tv_weight=1e-2, loss_func = SynthesisLoss4):
    ...
    for i in range(len(intervals)):
        ...
        output_image = run_synthesis(layer, image1, init_image, num_steps=num_steps, area = area, ver=ver, tv_weight=tv_weight,
                                     loss_func = loss_func, vec_mag=intervals[i], interpol_image=image2);
        if save_path is not None:
            if loss_func is SynthesisLoss4:
...
if __name__ == '__main__':
    ...
    for i in range(5):
        ...
        outputs = run_neural_interpolation('rocks', 'leaves', uid=f's{i+1}', num_steps=5000, input_image_dir = input_image_dir, loss_func = SynthesisLoss4, layer=args.layer, area=args.area);
